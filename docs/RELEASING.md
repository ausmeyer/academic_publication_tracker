...
When using Xcode’s signed-in Apple Account to upload a Developer ID archive, preserve the exact app under the archive’s `Submissions/<submission-id>/` directory: Xcode can re-sign it during export. After approval, attach and validate its ticket with `xcrun stapler staple`, then run `node scripts/verify-mac-signatures.mjs --distribution` against that app. Package the approved app with `electron-builder --prepackaged` outside synced folders, without rebuilding or re-signing it. Verify the finished DMG and ZIP and the extracted app before uploading. ZIPs retain the app’s stapled ticket; a ZIP itself cannot be stapled.
